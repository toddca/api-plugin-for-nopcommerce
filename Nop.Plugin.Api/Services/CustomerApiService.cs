// // -----------------------------------------------------------------------
// // <copyright from="2020" to="2020" file="CustomerApiService.cs" company="Lindell Management">
    ...
    public class CustomerApiService : ICustomerApiService
    {
            ...
            int limit = Constants.Configurations.DefaultLimit)
        {
            ...
            if (searchParams != null)
            {
                ...
                result = HandleCustomerGenericAttributes(searchParams,
                                                         query,
                                                         limit,
                                                         page,
                                                         order);
        ...
        private IQueryable<IGrouping<int, CustomerAttributeMappingDto>> GetCustomerAttributesMappingsByKey(
            IQueryable<IGrouping<int, CustomerAttributeMappingDto>> customerAttributesGroups,
            string key,
            string value)
        {
        ...
        private IQueryable<Customer> GetCustomersQuery(DateTime? createdAtMin = null, DateTime? createdAtMax = null, int sinceId = 0)
        {
            ...
            query = query.Where(customer =>
                                    !_customerService.GetCustomerRoles(customer, false).Any(r => r.Active && r.SystemName == NopCustomerDefaults.GuestsRoleName)
        ...
        private int GetDefaultStoreLangaugeId()
        {
            ...
            if (defaultLanguageId == 0)
            {
                ...
                var storeLanguages = allLanguages.Where(l =>
                                                            _storeMappingService.Authorize(l, _storeContext.CurrentStore.Id))
                                                 .ToList();
